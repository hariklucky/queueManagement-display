...
# 先删掉可能过旧/错误的 AppImage 自带库，再按麒麟兼容策略重装
rm -f "$APP_DIR/usr/lib/libfreetype.so.6" "$APP_DIR/usr/lib/libgbm.so.1"
kylin_ensure_freetype_for_webkit "$APP_DIR"
kylin_ensure_gbm_for_webkit "$APP_DIR"
kylin_copy_webkit_helpers "$APP_DIR"
kylin_patch_webkit_helpers "$APP_DIR" "/opt/qms/usr/lib/glibc-compat/$LD_LINUX"
...
find /opt/qms/usr/lib -path '*/webkit2gtk-4.1/*' -type f -exec chmod +x {} + 2>/dev/null || true
